...
const Alumnus_Card_New = ({ index, data }) => {
    ...
    return (
        <>
            {(data) && (
                <>
                    <m.div variants={_Transition_Card} initial="initial" animate="animate"
                        layoutId={`modal_${id}`} layout className="card bg-base-200 hover:bg-base-300 shadow select-none transition-colors">
                        <label htmlFor={`modal_${id}`} className="modal-button cursor-pointer">
